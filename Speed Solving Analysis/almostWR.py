...
def res(comp, bestmax):
    eve = three[three.competitionId == comp].reset_index(drop="index")
...
for comp in comps:
    ...
    if count % 1000 == 0:
        ...
        s.to_csv(f"unlucky {count}.csv", index=False)
...
stat.to_csv("unlucky.csv", index=False)
