...
def ordercompdf(competitions=competitions):
...
if __name__ == "__main__":
    for event in meanevents:
                        ...
                        for i in range(l):
                            if df.average[i] < wravg:
                                ...
                                # If there are no DNFs in the average
                                    ...
                                    mint.append(t / 100)
                    # MO3 Format
                    elif event in meanevents:
                        for i in range(l):
                            if 0 < df.average[i] < wravg:
                                wravg = df.average[i]
                            else:
                                a = df["value1"][i]
                                a = a * 100
                                b = df["value2"][i]
                                b = b * 100
                                if a > 0 and b > 0 and a + b < 3 * wravg:
                                    if event != "333fm" or event == "333fm" and 3 * wravg - a - b > 1500:
                                        wcaid = df.personId[i]
                                        # First DF
                                        if wcaid not in main_dict.keys():
                                            main_dict[wcaid] = 1
        ...
        if event not in meanevents:
            ...
            mc = pd.DataFrame(
                {"Name": wcaname(wcaids), "Country": wcacountry(wcaids), "Competition": komp, "Minimum Time Needed": mint,
                 "Current WR": cwr, "Solve1": v1, "Solve2": v2})
